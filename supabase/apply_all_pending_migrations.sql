-- ============================================================
-- COMBINED MIGRATION SCRIPT — All Pending Fixes
-- ============================================================
-- Generated: 2026-02-27
-- Purpose: Paste this entire file into Supabase SQL Editor and run.
-- Order: migrations are arranged in dependency order.
--
-- What this applies:
--   1. CRITICAL-1: Replace auth.uid() IS NULL with session-validated RLS
--   2. CRITICAL-2: Restrict tm-photos bucket (no more public access)
--   3. CRITICAL-3: Add company_id to project_shares
--   4. Fix foreman RLS (daily_reports UPDATE, disposal_loads cleanup)
--   5. Fix validate_field_session STABLE violation (remove UPDATE)
--   6. Fix draw_requests FK (ON DELETE SET NULL)
--   7. Fix project cascade delete (t_and_m_tickets, daily_reports, crew_checkins)
--   8. Add photos column to daily_reports
--   9. Fix admin project delete (access_level instead of legacy role)
--  10. Punch list field access (SELECT/INSERT/UPDATE for anon)
--
-- SAFE TO RE-RUN: All statements use IF NOT EXISTS, DROP IF EXISTS,
-- or CREATE OR REPLACE, making this fully idempotent.
-- ============================================================

BEGIN;

-- ============================================================
-- SECTION 1: RATE LIMITING (prerequisite for session creation)
-- ============================================================
...
-- ============================================================
-- SECTION 2: FIELD SESSIONS TABLE
-- ============================================================
...
-- ============================================================
-- SECTION 3: SESSION VALIDATION FUNCTIONS
-- (validate_field_session is the STABLE-safe version — no UPDATE)
-- ============================================================
...
-- ============================================================
-- SECTION 4: PIN → SESSION CREATION FUNCTION
-- ============================================================
...
-- ============================================================
-- SECTION 5: CENTRAL ACCESS HELPER
-- ============================================================
...
-- ============================================================
-- SECTION 6: REPLACE ALL auth.uid() IS NULL POLICIES
-- ============================================================
...
-- DAILY REPORTS (with UPDATE policy)
...
-- DISPOSAL LOADS (full cleanup of all legacy policy names)
...
-- ============================================================
-- SECTION 7: SESSION UTILITY FUNCTIONS
-- ============================================================
...
-- ============================================================
-- SECTION 8: CRITICAL-2 — Restrict tm-photos bucket
-- ============================================================
...
-- ============================================================
-- SECTION 9: CRITICAL-3 — Add company_id to project_shares
-- ============================================================
...
DECLARE
  null_count INTEGER;
BEGIN
  SELECT COUNT(*) INTO null_count FROM project_shares WHERE company_id IS NULL;
  IF null_count = 0 THEN
    EXECUTE 'ALTER TABLE project_shares ALTER COLUMN company_id SET NOT NULL';
  ELSE
    RAISE NOTICE 'WARNING: % project_shares rows still have NULL company_id — skipping NOT NULL', null_count;
  END IF;
END $$;
...
-- ============================================================
-- SECTION 10: Fix draw_requests FK
-- ============================================================
...
-- ============================================================
-- SECTION 11: Fix project cascade delete
-- ============================================================
...
-- Also add photos as TEXT[] for daily_reports (used by different code paths)
ALTER TABLE daily_reports
  ADD COLUMN IF NOT EXISTS photos TEXT[] DEFAULT '{}';

-- t_and_m_tickets FK → CASCADE
DO $$
DECLARE
  v_constraint TEXT;
BEGIN
...
-- crew_checkins FK → CASCADE (if table exists)
DO $$
DECLARE
  v_constraint TEXT;
...
-- ============================================================
-- SECTION 12: Fix admin project delete (access_level check)
-- ============================================================
  ...
  (company_id IS NOT NULL AND EXISTS (
    ...
    WHERE uc.user_id = auth.uid()
      ...
      AND (
        ...
        OR uc.role IN ('admin', 'owner', 'office')
      )
  ))
...
-- ============================================================
-- SECTION 13: Punch list field access
-- ============================================================
...
BEGIN
  ...
  RAISE NOTICE '============================================================';
  RAISE NOTICE '';
  RAISE NOTICE 'CRITICAL-1: auth.uid() IS NULL policies replaced with session validation';
  RAISE NOTICE 'CRITICAL-2: tm-photos bucket restricted (no more public access)';
  RAISE NOTICE 'CRITICAL-3: company_id added to project_shares';
  RAISE NOTICE '';
  RAISE NOTICE 'Also applied:';
  RAISE NOTICE '  - Foreman RLS fixes (daily reports UPDATE, disposal loads)';
  RAISE NOTICE '  - validate_field_session STABLE fix (no more UPDATE inside)';
  RAISE NOTICE '  - draw_requests FK fix (ON DELETE SET NULL)';
  RAISE NOTICE '  - Project cascade delete (t_and_m_tickets, daily_reports, crew_checkins)';
  RAISE NOTICE '  - Photos column on daily_reports';
  RAISE NOTICE '  - Admin project delete (access_level check)';
  RAISE NOTICE '  - Punch list field access (SELECT/INSERT/UPDATE for anon)';
  RAISE NOTICE '';
  RAISE NOTICE 'MANUAL ACTION REQUIRED:';
  RAISE NOTICE '  Storage > tm-photos > Edit bucket > uncheck "Public bucket"';
  RAISE NOTICE '============================================================';
END $$;
