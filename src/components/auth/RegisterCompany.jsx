  ...
  const handleRegisterCompany = async () => {
    ...
    try {
      ...
      if (userAlreadyExists) {
        ...
        // With Supabase email confirmation on, signUp returns no session. The
        // register_company RPC is GRANT EXECUTE ... TO authenticated, so
        // calling it now would fail with insufficient privilege. Ask the user
        // to confirm and retry — on the second attempt signUp reports the
        // user already exists and the branch above signs them in, then the
        // RPC below runs with a real authenticated session and succeeds.
        if (!authData.session) {
          onShowToast(
            `Check ${normalizedEmail} for a confirmation link. Once confirmed, submit this form again to finish creating your company.`,
            'info'
          )
          return
        }
