...
export default function DumpSitesSection({ company, onShowToast }) {
  ...
  const [pendingRates, setPendingRates] = useState({}) // Track pending rate values
  ...
  // Debounced rate save - waits 500ms after last keystroke
  ...
  const getRate = (site, wasteType) => {
    ...
    // Show pending value if exists, otherwise show saved value
  ...
  if (loading) {
    return <div className="loading">Loading dump sites...</div>
  }
  ...
  return (
    <div className="pricing-section">
      <div className="dump-site-header">
        <div>
          <h2>Disposal & Haul-Off</h2>
          <p className="section-desc">
            Configure dump sites and estimated costs per load for each waste type.
            Use negative values for recyclables (metal, copper) to indicate revenue.
          </p>
        </div>
        <button
