...
export async function exportCORToPDF(cor, project, company, branding = {}, tmTickets = null, options = {}) {
  ...
  if (tmTickets && tmTickets.length > 0) {
    ...
    doc.text('Time & Materials:', summaryCol1, yPos)
    doc.setFont('helvetica', 'bold')
    ...
    for (const ticket of tmTickets) {
      ...
      doc.setFont('helvetica', 'bold')
      doc.setTextColor(30, 41, 59)
      doc.text(`TIME & MATERIALS — ${formatDate(ticket.work_date)}`, margin + 10, yPos + 8)
      ...
      if (ticket.t_and_m_items?.length > 0) {
        ...
        doc.setFont('helvetica', 'normal')
        doc.setTextColor(100, 116, 139)
        doc.text('Source: Time & Materials', margin + 55, yPos)
        yPos += 6
...
export async function exportTMTicketToPDF(ticket, project, company, branding = {}) {
  ...
  // Time & Materials Ticket title
  doc.setFontSize(16)
  doc.setFont('helvetica', 'bold')
  doc.setTextColor(30, 41, 59)
  doc.text('TIME & MATERIALS TICKET', pageWidth - margin, yPos + 8, { align: 'right' })
