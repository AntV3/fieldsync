  ...
  const projectAllocation = projects
    ...
    .map(project => {
      ...
      for (const checkin of projectCheckins) {
        ...
        dailyCounts[date] += checkin.worker_count ?? (checkin.workers?.length ?? 0)
      }
  ...
  for (let w = -3; w <= 0; w++) {
    ...
    for (const checkin of crewCheckins) {
      ...
      if (checkinDate >= weekStart && checkinDate < weekEnd) {
        ...
        projectCrews[checkin.project_id].totalWorkers += checkin.worker_count ?? (checkin.workers?.length ?? 0)
